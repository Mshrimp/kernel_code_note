 ...
 */
#if 1
	con_reg = ioremap(LED_CON_REG, 8);
	dat_reg = ioremap(LED_DAT_REG, 8);

	iowrite32(ioread32(con_reg) & (~0xFFFF), con_reg);
	iowrite32(ioread32(con_reg) | (0x1111), con_reg);
#endif

	iowrite32(ioread32(dat_reg) | 0xF, dat_reg);
}
...
//	gpio_direction_output(tdev->gpio[on], 0);
	iowrite32(ioread32(dat_reg) & (~(0x1 << on)), dat_reg);
}
...
int test_init(void)			
{
	...
	//ret = register_chrdev_region(tdev.major, 4, DEV_NAME);
	if(ret < 0)
	{
		return ret;
	}
